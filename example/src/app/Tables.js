...
class Tables extends Component {
	...
	onSearchChange = (event) => {
		...
		if(value === "") {
			...
			newFiltered = tableValues.filter(function(tv, index){
				return (tv[0].indexOf(value) > -1 || tv[1].indexOf(value) > -1 || tv[2].indexOf(value) > -1 || tv[3].indexOf(value) > -1)
			});
	...
	getTablePageButtons = (pages) => {
		var buttons = [];
		buttons.push(<Button classes="text plain btn-small" onClick={this.previousPage}>Previous</Button>)
		for(var i = 0; i < pages; i ++) {
			var color = i === this.state.tablePage ? "information" : "text plain";
			(function(th){
				var index = i;
				buttons.push(<Button classes={color + " btn-small"} onClick={() => th.updatePage(index)}>{i+1}</Button>)
			})(this);
		}
		buttons.push(<Button classes="text plain btn-small" onClick={this.nextPage}>Next</Button>)
		return buttons;
	}
	...
	render() {
		...
		var pages = this.state.filteredValues.length / this.state.tableLimit;
		if(tableEnd > this.state.filteredValues.length - 1) tableEnd = this.state.filteredValues.length;
					...
					<Table 
						...
						title={
							<div className="grid grid-2 vertical-align">
								...
								<div>
									<Input classes="small" placeholder="Search..." onChange={this.onSearchChange} />
								</div>
							</div>
						}
						...
						footer={<div className="grid grid-2 vertical-align">
									<div>
										Showing {tableStart} to {tableEnd} of {this.state.filteredValues.length} entries
										{this.state.searchText && " (filtered from "+tableValues.length+")"}
									</div>
